...
public class SubjectDao extends AbstractDao<Subject, Long> {
    ...
    public List<Subject> findByCourseAndGroupAndDateAndIsNumerator_NOW(Course course, Group group, LocalDateTime localDateTime, boolean isNumerator) {
        ...
        Query<Subject> query = session.createQuery("from Subject where course=:paramCourse AND group=:paramGroup AND (numeratorOrDenominator=0 OR numeratorOrDenominator=:paramNumeratorOrDenominator)" +
